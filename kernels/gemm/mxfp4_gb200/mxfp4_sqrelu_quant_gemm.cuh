...
namespace mxfp4_sqrelu_quant_gemm {
...
__device__ __noinline__ void stage_sqrelu_pairs(
    subtile_rt& D_fl,
    bf16_2 (*pairs)[33],
    int lane_id)
{
    ...
    for (int i = 0; i < subtile_rt::height; i++) {
        ...
        for (int j = 0; j < subtile_rt::width; j++) {
            ...
            const float h0_x = __bfloat162float(__float2bfloat16_rn(D_fl.tiles[i][j].data[0].x * MXFP4_ALPHA));
            const float h0_y = __bfloat162float(__float2bfloat16_rn(D_fl.tiles[i][j].data[0].y * MXFP4_ALPHA));
            const float h1_x = __bfloat162float(__float2bfloat16_rn(D_fl.tiles[i][j].data[1].x * MXFP4_ALPHA));
            const float h1_y = __bfloat162float(__float2bfloat16_rn(D_fl.tiles[i][j].data[1].y * MXFP4_ALPHA));
            const float h2_x = __bfloat162float(__float2bfloat16_rn(D_fl.tiles[i][j].data[2].x * MXFP4_ALPHA));
            const float h2_y = __bfloat162float(__float2bfloat16_rn(D_fl.tiles[i][j].data[2].y * MXFP4_ALPHA));
            const float h3_x = __bfloat162float(__float2bfloat16_rn(D_fl.tiles[i][j].data[3].x * MXFP4_ALPHA));
            const float h3_y = __bfloat162float(__float2bfloat16_rn(D_fl.tiles[i][j].data[3].y * MXFP4_ALPHA));

            const float r0_x = fmaxf(h0_x, 0.0f);
            const float r0_y = fmaxf(h0_y, 0.0f);
            const float r1_x = fmaxf(h1_x, 0.0f);
            const float r1_y = fmaxf(h1_y, 0.0f);
            const float r2_x = fmaxf(h2_x, 0.0f);
            const float r2_y = fmaxf(h2_y, 0.0f);
            const float r3_x = fmaxf(h3_x, 0.0f);
            const float r3_y = fmaxf(h3_y, 0.0f);

            pairs[pair_base][i * 16 + row_pair_idx] = bf16_2{
                __float2bfloat16_rn(r0_x * r0_x),
                __float2bfloat16_rn(r0_y * r0_y)};
            pairs[pair_base][i * 16 + row_pair_idx + 8] = bf16_2{
                __float2bfloat16_rn(r1_x * r1_x),
                __float2bfloat16_rn(r1_y * r1_y)};
            pairs[pair_base + 4][i * 16 + row_pair_idx] = bf16_2{
                __float2bfloat16_rn(r2_x * r2_x),
                __float2bfloat16_rn(r2_y * r2_y)};
            pairs[pair_base + 4][i * 16 + row_pair_idx + 8] = bf16_2{
                __float2bfloat16_rn(r3_x * r3_x),
                __float2bfloat16_rn(r3_y * r3_y)};
        }
    }
}
    ...
    if (warpgroup_id >= C::CONSUMER_WARPGROUPS && warp::elect_leader()) {
        ...
        for (int block_idx = cluster_id; block_idx < num_blocks; block_idx += gridDim.x / C::CLUSTER_SIZE) {
            ...
            for (int epi = 0; epi < C::EPI_PIPE_DEPTH; epi++) {
                ...
                stage_sqrelu_pairs<C>(D_regs_fl[epi], pairs, lane_id);
                __syncwarp();
