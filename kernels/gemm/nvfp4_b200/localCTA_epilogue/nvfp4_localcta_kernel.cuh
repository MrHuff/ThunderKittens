...
__device__ inline void scale_shared_fp8_tile(st_fp8e4m3<ROWS, COLS, false> &tile, float scale) {
    ...
    #pragma unroll
    for (int idx = 0; idx < ROWS * COLS; idx += 4) {
        uint32_t packed;
        uint8_t *bytes = reinterpret_cast<uint8_t*>(&packed);
        #pragma unroll
        for (int j = 0; j < 4; ++j) {
            const fp8e4m3 scaled = fp8e4m3(static_cast<float>(vals[idx + j]) * scale);
            bytes[j] = *reinterpret_cast<const uint8_t*>(&scaled);
        }
        const uint32_t smem_addr = static_cast<uint32_t>(__cvta_generic_to_shared(vals + idx));
            ...
            for (int block_idx = cluster_id; block_idx < num_blocks; block_idx += gridDim.x / C::CLUSTER_SIZE) {
                ...
                for (int i = 0; i < num_red_blocks; ++i) {
                    ...
                    for (int ii = 0; ii < C::MMA_PER_TILE; ii++) {
                        ...
                        const float a_chunk_sg = g.A_sg_chunks[a_chunk_row * g.A_sg_stride + chunk_k];
                        ...
                        const float b_chunk_sg_0 = g.B_sg_chunks[b_chunk_row_0 * g.B_sg_stride + chunk_k];
                        ...
                        if constexpr (C::B_SC_SIZE == 2) {
                            const float b_chunk_sg_1 = g.B_sg_chunks[(b_chunk_row_0 + 1) * g.B_sg_stride + chunk_k];
