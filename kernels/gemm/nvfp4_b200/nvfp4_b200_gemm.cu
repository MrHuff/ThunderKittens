...
__global__ void v5_sum3_rmsnorm_bwd_dx_kernel(
    ...
    for (int64_t col = tid; col < K; col += blockDim.x) {
        const int64_t idx = base + col;
        const __nv_bfloat16 dy_bf16 = __hadd(__hadd(d0[idx], d1[idx]), d2[idx]);
        d_sum[idx] = dy_bf16;
        const float dy = __bfloat162float(dy_bf16);
        const float x = __bfloat162float(input[idx]);
        const float gamma = __bfloat162float(norm_weight[col]);
        partial += dy * gamma * x;
    }
    ...
    for (int64_t col = tid; col < K; col += blockDim.x) {
        const int64_t idx = base + col;
        const float dy = __bfloat162float(d_sum[idx]);
        const float x = __bfloat162float(input[idx]);
        const float gamma = __bfloat162float(norm_weight[col]);
        const float dx = inv * (dy * gamma) - x * inv3_dot;
        grad_input[idx] = __float2bfloat16(dx);
...
template <int COLS>
__global__ void v5_rmsnorm_bwd_dgamma_tiled_kernel(
    const __nv_bfloat16* __restrict__ d_normed,
    const __nv_bfloat16* __restrict__ input,
    const float* __restrict__ inv_rms,
    float* __restrict__ dgamma,
    ...
    static_assert(COLS == 2 || COLS == 4 || COLS == 8 || COLS == 16 || COLS == 32);
...
void nvfp4_sum3_rmsnorm_bwd_entrypoint(
    const at::Tensor& d0,
    const at::Tensor& d1,
    const at::Tensor& d2,
    const at::Tensor& input,
    ...
    TORCH_CHECK(d0.is_cuda(), "d0 must be CUDA");
    kittens::py::device_check(
        d0, d1, d2, input, norm_weight, inv_rms, d_sum, grad_input, dgamma);
    TORCH_CHECK(d0.is_contiguous() && d0.scalar_type() == at::kBFloat16,
                "d0 must be contiguous CUDA bf16");
    TORCH_CHECK(d1.is_contiguous() && d1.scalar_type() == at::kBFloat16,
                "d1 must be contiguous CUDA bf16");
    TORCH_CHECK(d2.is_contiguous() && d2.scalar_type() == at::kBFloat16,
                "d2 must be contiguous CUDA bf16");
    TORCH_CHECK(input.is_contiguous() && input.scalar_type() == at::kBFloat16,
                "input must be contiguous CUDA bf16");
    TORCH_CHECK(norm_weight.is_contiguous() && norm_weight.scalar_type() == at::kBFloat16,
                "norm_weight must be contiguous CUDA bf16");
    TORCH_CHECK(inv_rms.is_contiguous() && inv_rms.scalar_type() == at::kFloat,
                "inv_rms must be contiguous CUDA fp32");
    TORCH_CHECK(d0.dim() == 2, "d0 must be rank-2");
    const int64_t M = d0.size(0);
    const int64_t K = d0.size(1);
    TORCH_CHECK(M > 0 && K > 0, "M and K must be positive");
    TORCH_CHECK(d1.sizes() == at::IntArrayRef({M, K})
                    && d2.sizes() == at::IntArrayRef({M, K}),
                "d1 and d2 shape mismatch");
    TORCH_CHECK(input.sizes() == at::IntArrayRef({M, K}), "input shape mismatch");
    TORCH_CHECK(norm_weight.dim() == 1 && norm_weight.numel() == K,
                "norm_weight must have shape [K]");
    TORCH_CHECK(inv_rms.dim() == 1 && inv_rms.numel() == M,
                "inv_rms must have shape [M]");
    TORCH_CHECK(d_sum.is_contiguous() && d_sum.scalar_type() == at::kBFloat16
                    && d_sum.sizes() == at::IntArrayRef({M, K}),
                "d_sum must be contiguous CUDA bf16 [M,K]");
    TORCH_CHECK(grad_input.is_contiguous() && grad_input.scalar_type() == at::kBFloat16
                    && grad_input.sizes() == at::IntArrayRef({M, K}),
                "grad_input must be contiguous CUDA bf16 [M,K]");
    ...
    auto stream = at::cuda::getCurrentCUDAStream();
    constexpr int threads = 256;
    v5_sum3_rmsnorm_bwd_dx_kernel<<<static_cast<int>(M), threads, 0, stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(d0.data_ptr()),
        reinterpret_cast<const __nv_bfloat16*>(d1.data_ptr()),
        reinterpret_cast<const __nv_bfloat16*>(d2.data_ptr()),
        reinterpret_cast<const __nv_bfloat16*>(input.data_ptr()),
        reinterpret_cast<const __nv_bfloat16*>(norm_weight.data_ptr()),
        reinterpret_cast<const float*>(inv_rms.data_ptr()),
        reinterpret_cast<__nv_bfloat16*>(d_sum.data_ptr()),
        reinterpret_cast<__nv_bfloat16*>(grad_input.data_ptr()),
        M,
        K);
    const auto* d_sum_ptr = reinterpret_cast<const __nv_bfloat16*>(d_sum.data_ptr());
    const auto* input_ptr = reinterpret_cast<const __nv_bfloat16*>(input.data_ptr());
    const auto* inv_rms_ptr = reinterpret_cast<const float*>(inv_rms.data_ptr());
    auto* dgamma_ptr = reinterpret_cast<float*>(dgamma.data_ptr());
    constexpr int dgamma_columns = 8;
    const int64_t dgamma_blocks = (K + dgamma_columns - 1) / dgamma_columns;
    v5_rmsnorm_bwd_dgamma_tiled_kernel<dgamma_columns><<<
        static_cast<int>(dgamma_blocks), threads, 0, stream>>>(
            d_sum_ptr, input_ptr, inv_rms_ptr, dgamma_ptr, M, K);
    CUDACHECK(cudaGetLastError());
}
...
void check_nvfp4_residual_gemm_inputs(
    const at::Tensor &A,
    const at::Tensor &A_sc,
    const at::Tensor &A_sc_global,
    const at::Tensor &B,
    ...
    const auto check_tma_alignment = [](const at::Tensor &tensor, const char *name) {
        TORCH_CHECK(
            (reinterpret_cast<uintptr_t>(tensor.data_ptr()) & 0xF) == 0,
            "v5 residual ", name, " data pointer must be 16-byte aligned"
        );
    };
    const auto check_fp4 = [](const at::Tensor &tensor, const char *name) {
        TORCH_CHECK(tensor.is_cuda(), "v5 residual ", name, " must be CUDA");
        TORCH_CHECK(tensor.is_contiguous(), "v5 residual ", name, " must be contiguous");
    ...
    TORCH_CHECK(A.size(1) == B.size(1), "v5 residual A and B must share packed K");
    TORCH_CHECK(
        M > 0 && N > 0 && K > 0 &&
    ...
    const auto check_scale = [K](
        ...
        TORCH_CHECK(
            ...
            "v5 residual ", name, " must be prepared [rows/128,K/64,512]"
        );
    };
    ...
    const auto check_global = [](const at::Tensor &tensor, const char *name) {
        ...
        TORCH_CHECK(tensor.is_contiguous(), "v5 residual ", name, " must be contiguous");
        TORCH_CHECK(
            tensor.scalar_type() == at::kFloat && tensor.dim() == 1 && tensor.numel() == 1,
            "v5 residual ", name, " must be contiguous float32 [1]"
        );
    };
    check_global(A_sc_global, "A_sg");
    check_global(B_sc_global, "B_sg");
    for (const auto &[tensor, name] : std::array<std::pair<const at::Tensor*, const char*>, 2>{
             std::pair{&R, "R"}, std::pair{&D, "D"}}) {
        TORCH_CHECK(tensor->is_cuda(), "v5 residual ", name, " must be CUDA");
        TORCH_CHECK(tensor->is_contiguous(), "v5 residual ", name, " must be contiguous");
        TORCH_CHECK(tensor->scalar_type() == at::kBFloat16,
                    "v5 residual ", name, " must be bf16");
        TORCH_CHECK(tensor->dim() == 2 && tensor->size(0) == M && tensor->size(1) == N,
    ...
    c3_row_scale::check_output_no_overlap(D, A_sc_global, "A_sg");
    c3_row_scale::check_output_no_overlap(D, B, "B");
    c3_row_scale::check_output_no_overlap(D, B_sc, "B_sc");
    c3_row_scale::check_output_no_overlap(D, B_sc_global, "B_sg");
    c3_row_scale::check_output_no_overlap(D, R, "R");
}
...
void nvfp4_gemm_residual_entrypoint(
    ...
    kittens::py::device_check(A, A_sc, A_sc_global, B, B_sc, B_sc_global, R, D);
...
static void run_gemm_residual_with_config(
    ...
    // launch_kernel's attribute cache is process-global, while CUDA function
    // attributes are device-specific. Keep this new entrypoint valid when a
    // process first uses the same specialization on a different GPU.
    struct ResidualAttributeCache {
        std::atomic<int> max_dynamic_smem[64];
        std::mutex locks[64];
        ResidualAttributeCache() {
            for (auto &value : max_dynamic_smem) {
                value.store(-1, std::memory_order_relaxed);
            }
        }
    };
    static ResidualAttributeCache attribute_cache;
    ...
    if (dynamic_smem > attribute_cache.max_dynamic_smem[device].load(std::memory_order_acquire)) {
        std::lock_guard<std::mutex> lock(attribute_cache.locks[device]);
        if (dynamic_smem > attribute_cache.max_dynamic_smem[device].load(std::memory_order_relaxed)) {
            CUDACHECK(cudaFuncSetAttribute(
                kittens::py::global_kernel<C, G, nvfp4_gemm::kernel<C>>,
                cudaFuncAttributeMaxDynamicSharedMemorySize,
                dynamic_smem));
...
// ================================================================
template <typename C, typename G, auto Kernel>
void ensure_batched_accum_kernel_attributes(const G &g) {
    static_assert(
        C::CLUSTER_SIZE <= 8,
        "batched accumulator requires portable cluster dimensions");
    struct PerDeviceCache {
        std::array<std::atomic<int>, C10_COMPILE_TIME_MAX_GPUS> max_smem;
        std::mutex mutex;
    ...
    TORCH_CHECK(
        device >= 0 && device < C10_COMPILE_TIME_MAX_GPUS,
        "batched accumulator CUDA device index is out of range");
    const int required_smem = g.dynamic_shared_memory();
    auto &configured_smem = cache.max_smem[device];
    if (required_smem <= configured_smem.load(std::memory_order_acquire)) {
        return;
...
void check_batched_accum_output_no_overlap(
    const at::Tensor &output,
    const at::Tensor &input,
    const char *input_name
) {
    const auto output_begin = reinterpret_cast<uintptr_t>(output.data_ptr());
    const auto input_begin = reinterpret_cast<uintptr_t>(input.data_ptr());
    const auto output_bytes = static_cast<uintptr_t>(output.nbytes());
    const auto input_bytes = static_cast<uintptr_t>(input.nbytes());
    TORCH_CHECK(
        output_begin + output_bytes <= input_begin ||
            input_begin + input_bytes <= output_begin,
        "batched accumulation D_out must not overlap ", input_name);
}
...
void nvfp4_batched_accum_gemm_entrypoint(
    ...
    TORCH_CHECK(
        ...
        B_sc_list.size() == static_cast<size_t>(n) &&
        B_sg_list.size() == static_cast<size_t>(n),
        "all batched accumulation input lists must have the same length");
    TORCH_CHECK(
        D_out.is_cuda() && D_out.is_contiguous() && D_out.dim() == 2 &&
        D_out.scalar_type() == at::kBFloat16,
        "D_out must be contiguous CUDA bfloat16 [M,N]");
    ...
    TORCH_CHECK(
        M > 0 && N_out > 0 && K > 0 &&
        M % 256 == 0 && N_out % 256 == 0 && K % 256 == 0,
        "batched accumulation M,N,K must be positive multiples of 256");
    for (int i = 0; i < n; ++i) {
        TORCH_CHECK(
            A_list[i].dim() == 2 && B_list[i].dim() == 2 &&
            A_list[i].size(0) == M && B_list[i].size(0) == N_out &&
            A_list[i].size(1) == B_list[i].size(1),
            "each A/B pair must be rank 2 with shapes [M,Ki/2] and [N,Ki/2]");
        ...
        TORCH_CHECK(
            A_sc_list[i].is_cuda() && A_sc_list[i].is_contiguous() &&
            A_sc_list[i].scalar_type() == at::kFloat8_e4m3fn &&
            A_sc_list[i].numel() == M * K_i / 16,
            "A_sc_list entries must be contiguous CUDA fp8 with M*K/16 elements");
        TORCH_CHECK(
            B_sc_list[i].is_cuda() && B_sc_list[i].is_contiguous() &&
            B_sc_list[i].scalar_type() == at::kFloat8_e4m3fn &&
            B_sc_list[i].numel() == N_out * K_i / 16,
            "B_sc_list entries must be contiguous CUDA fp8 with N*K/16 elements");
        TORCH_CHECK(
            A_sg_list[i].is_cuda() && A_sg_list[i].is_contiguous() &&
            A_sg_list[i].scalar_type() == at::kFloat && A_sg_list[i].numel() == 1,
            "A_sg_list entries must be contiguous CUDA float32 scalars");
        TORCH_CHECK(
            ...
            "B_sg_list entries must be contiguous CUDA float32 scalars");
        kittens::py::device_check(
            D_out, A_list[i], A_sc_list[i], A_sg_list[i],
            B_list[i], B_sc_list[i], B_sg_list[i]);
        check_batched_accum_output_no_overlap(D_out, A_list[i], "A_list");
        check_batched_accum_output_no_overlap(D_out, A_sc_list[i], "A_sc_list");
        check_batched_accum_output_no_overlap(D_out, A_sg_list[i], "A_sg_list");
        check_batched_accum_output_no_overlap(D_out, B_list[i], "B_list");
        check_batched_accum_output_no_overlap(D_out, B_sc_list[i], "B_sc_list");
        check_batched_accum_output_no_overlap(D_out, B_sg_list[i], "B_sg_list");
    }
    c10::cuda::CUDAGuard device_guard(D_out.device());
    ...
    auto build_and_launch = [&]<typename C>() {
        ...
        bool use_fixed2_8b_ffn =
            n == 2 && M == 32768 && N_out == 4096 &&
            2 * A_list[0].size(1) == 14336;
        for (int i = 1; i < n && use_fixed2_8b_ffn; ++i) {
            use_fixed2_8b_ffn = 2 * A_list[i].size(1) == 14336;
        }
        if (use_fixed2_8b_ffn) {
            ensure_batched_accum_kernel_attributes<
                C, G, nvfp4_batched_accum_gemm::kernel_fixed2<C>>(g_host);
            kittens::py::launch_kernel<
                C, G, nvfp4_batched_accum_gemm::kernel_fixed2<C>>(g_host);
        } else {
            ensure_batched_accum_kernel_attributes<
                C, G, nvfp4_batched_accum_gemm::kernel<C>>(g_host);
            kittens::py::launch_kernel<
                C, G, nvfp4_batched_accum_gemm::kernel<C>>(g_host);
        }
    };
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("c5_rms_bwd_partial_reduce_apply_dx",
          &c5_rms_bwd::partial_dot_reduce_apply_dx_entrypoint,
          "C5 partial-dot, dot reduction, and dx apply in one launch",
          pybind11::arg("x"), pybind11::arg("dy"), pybind11::arg("coeff"),
          pybind11::arg("partial_dot"), pybind11::arg("dot"), pybind11::arg("dx"),
          pybind11::arg("hidden_size"), pybind11::arg("gamma") = std::nullopt);
    m.def("g4_rms_bwd_apply_dx_native_order", &c5_rms_bwd::apply_dx_native_order_entrypoint,
          ...
          pybind11::arg("dx"), pybind11::arg("hidden_size"),
          pybind11::arg("gamma") = std::nullopt);
    m.def("g4_rms_bwd_reduce_apply_dx_native_order",
          &c5_rms_bwd::reduce_dot_apply_dx_native_order_entrypoint,
          "G4 reduce dot and apply dx in the production native operation order",
          pybind11::arg("partial_dot"), pybind11::arg("x"), pybind11::arg("dy"),
          pybind11::arg("coeff"), pybind11::arg("dot"), pybind11::arg("dx"),
          pybind11::arg("hidden_size"), pybind11::arg("gamma") = std::nullopt);
