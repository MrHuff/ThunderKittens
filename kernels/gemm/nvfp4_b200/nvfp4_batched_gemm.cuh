...
//
// The default grid uses (spatial_tiles, 1, num_batches). The opt-in flattened
// schedule uses one persistent grid over all batch tiles so tensor-memory
// provisioning is amortized across experts with different shapes.
// ================================================================
namespace nvfp4_batched_gemm {

// Five 128-byte TMA descriptors plus scale pointers/strides and block counts
// are carried in the kernel argument block per batch. 40 entries remain below
...
struct globals {
    ...
    int       total_blocks;
    int       flatten_batches;
    int       batch_block_offsets[MAX_BATCHES + 1];
    int       num_red_blocks[MAX_BATCHES];
    int       num_row_blocks[MAX_BATCHES];
    int       num_col_blocks[MAX_BATCHES];
    ...
    __host__ inline dim3 grid() const {
        int spatial_tiles = total_blocks;
        if (!flatten_batches) {
            spatial_tiles = 0;
            for (int batch = 0; batch < num_batches; ++batch) {
                spatial_tiles = max(
                    spatial_tiles,
                    num_row_blocks[batch] * num_col_blocks[batch]
                );
            }
        }
        int grid_x = min(spatial_tiles, num_sms());
        grid_x = max(C::CLUSTER_SIZE, grid_x / C::CLUSTER_SIZE * C::CLUSTER_SIZE);
        return dim3(grid_x, 1, flatten_batches ? 1 : num_batches);
    }
...
template <typename C>
__device__ inline work_item locate_work_item(
    const globals<C> &g,
    int work_idx,
    int fixed_batch,
    int &batch_hint
) {
    int batch = fixed_batch;
    int block_idx = work_idx;
    if (g.flatten_batches) {
        batch = batch_hint;
        #pragma unroll 1
        while (work_idx >= g.batch_block_offsets[batch + 1]) {
            ++batch;
        }
        batch_hint = batch;
        block_idx = work_idx - g.batch_block_offsets[batch];
    }

    const int num_row_blocks = g.num_row_blocks[batch];
    const int num_col_blocks = g.num_col_blocks[batch];
    ...
    const int rows_in_supergroup = min(
        C::SUPERGROUP_SIZE,
    ...
    return {
        batch,
        g.num_red_blocks[batch],
        supergroup_idx * C::SUPERGROUP_SIZE + row_within_supergroup,
        idx_within_supergroup / rows_in_supergroup,
    };
}
...
__device__ inline void kernel(const globals<C> &g) {
    ...
    const int fixed_batch = blockIdx.z;
    const int num_work_items = g.flatten_batches
        ? g.total_blocks
        : g.num_row_blocks[fixed_batch] * g.num_col_blocks[fixed_batch];

    const int warpgroup_id = warpgroup::groupid();
    const int cta_id = cluster_ctarank();
    const int cluster_id = clusterIdx().x;
    const int work_stride = gridDim.x / C::CLUSTER_SIZE;
    uint32_t stage = 0;
    ...
    if (warpgroup_id >= C::CONSUMER_WARPGROUPS && warp::elect_leader()) {
        ...
        if (warp_id == 3) {
            ...
            int batch_hint = g.flatten_batches ? 0 : fixed_batch;
            for (int work_idx = cluster_id; work_idx < num_work_items; work_idx += work_stride) {
                const work_item work = locate_work_item(
                    g, work_idx, fixed_batch, batch_hint
                );
                tma_dev_proxy<typename G::A_fp4x2_gl> proxy_A(&g.A_tma[work.batch]);
                tma_dev_proxy<typename G::B_fp4x2_gl> proxy_B(&g.B_tma[work.batch]);

                for (int i = 0; i < work.num_red_blocks; ++i) {
                    wait(inputs_finished[stage], get_phasebit<1>(phasebits, stage));
                    tma::cluster::load_async(input_tiles[stage].A, proxy_A, {work.row_block_idx*2 + cta_id, i}, tiles_arrived[stage], (uint16_t)(1<<cta_id), 0);
                    tma::cluster::load_async(input_tiles[stage].B, proxy_B, {work.col_block_idx*2 + cta_id, i}, tiles_arrived[stage], (uint16_t)(1<<cta_id), 0);
            ...
            for (int work_idx = cluster_id; work_idx < num_work_items; work_idx += work_stride) {
                ...
                tma_dev_proxy<typename G::A_sc_gl> proxy_A_sc(&g.A_sc_tma[work.batch]);
                tma_dev_proxy<typename G::B_sc_gl> proxy_B_sc(&g.B_sc_tma[work.batch]);

                for (int i = 0; i < work.num_red_blocks; ++i) {
                    wait(inputs_finished[stage], get_phasebit<1>(phasebits, stage));
                    tma::cluster::load_async(input_scales[stage].A, proxy_A_sc, {work.row_block_idx*2 + cta_id, i, 0}, scales_arrived[stage], (uint16_t)(1<<cta_id), 0);
                    if constexpr (C::B_SC_SIZE == 2) tma::cluster::load_async(input_scales[stage].B[cta_id], proxy_B_sc, {work.col_block_idx*2 + cta_id, i, 0}, scales_arrived[stage], (uint16_t)(0b11), 0);
                    else if (cta_id == 0)            tma::cluster::load_async(input_scales[stage].B[0], proxy_B_sc, {work.col_block_idx, i, 0}, scales_arrived[stage], (uint16_t)(0b11), 0);
                    update_phasebit<1>(phasebits, stage);
                    stage = (stage + 1) % C::LOAD_PIPE_DEPTH;
                }
            }
        ...
        for (int work_idx = cluster_id; work_idx < num_work_items; work_idx += work_stride) {
            work_item work{};
            if (laneid() == 0) {
                work = locate_work_item(
                    g, work_idx, fixed_batch, batch_hint
                );
            }
            work.batch = __shfl_sync(MASK_ALL, work.batch, 0);
            work.num_red_blocks = __shfl_sync(
                MASK_ALL, work.num_red_blocks, 0
            );
            work.row_block_idx = __shfl_sync(
                MASK_ALL, work.row_block_idx, 0
            );
            work.col_block_idx = __shfl_sync(
                MASK_ALL, work.col_block_idx, 0
            );
            tma_dev_proxy<typename G::D_gl> proxy_D(&g.D_tma[work.batch]);
            float global_scale = 0.0f;
            if (laneid() == 0) {
                global_scale =
                    g.A_sg[work.batch][
                        work.row_block_idx * g.A_sg_stride[work.batch]
                    ] *
                    g.B_sg[work.batch][
                        work.col_block_idx * g.B_sg_stride[work.batch]
                    ];
            }
            global_scale = __shfl_sync(MASK_ALL, global_scale, 0);
            ...
            for (int i = 0; i < C::EPI_PIPE_DEPTH; i++) {
                ...
                warpgroup::sync(1);
                warpgroup::tma::store_async<dim::ROW, cache_policy::EVICT_FIRST>(proxy_D, output_tiles.D[i%C::NUM_D_TILES], {work.row_block_idx*2 + cta_id, C::EPI_PIPE_DEPTH*work.col_block_idx + i});
            }
            update_phasebit<0>(phasebits, 0);
        }
        warpgroup::sync(1);
        // The read wait only permits shared-memory reuse; it does not make the
        // TMA writes globally complete before the CTA exits.
        warpgroup::tma::store_async_wait<0>();
        if constexpr (C::USE_PDL) warpgroup::pdl::arrive();
        if (warpgroup::warpid() == 0) {
            if (warp::elect_leader()) {
                tma::cluster::arrive(tmem_finished, 1 - cta_id);
            }
