...
// BF16 mode: outputs BF16 grad_logits (identical interface to v1/v3)
template <typename C>
...
// FP4 mode: outputs quantized G in NVFP4 format (same as v3 for now)
template <typename C>
static void launch_backward_v4_fp4(
    const at::Tensor &A, const at::Tensor &A_sc, const at::Tensor &A_sc_global,
    const at::Tensor &B, const at::Tensor &B_sc, const at::Tensor &B_sc_global,
...
using bwd_v4_fp4_L4_SG8  = nvfp4_cce_backward_v4::config<4, 8, false, true>;

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("backward_v4_bf16_L4_SG8", &launch_backward_v4_bf16<bwd_v4_bf16_L4_SG8>,
          "NVFP4 CCE backward v4 (BF16 output) L4 SG8");
    m.def("backward_v4_fp4_L4_SG8", &launch_backward_v4_fp4<bwd_v4_fp4_L4_SG8>,
          "NVFP4 CCE backward v4 (FP4 output) L4 SG8");
}
#endif
